...
class DrawnProfileNaked:
    ...
    def __find_day_high_and_low_prices(self) -> tuple[int, int]:
        ...
        day_high = max([round(float(i[1]) * 100) for i in self.__mp_intervals])
        day_low = min([round(float(i[2]) * 100) for i in self.__mp_intervals])
    ...
    def __init__(self, mp_intervals, letters, ticker):
        ...
        """
        self.__mp_intervals = mp_intervals
        ...
        self.__mark_mp_poc()
        self.__market_profile_as_string = self.__convert_list_to_string()
    ...
    def __build_unfolded_and_collapsed_mp_without_center_and_poc(self) -> None:
        ...
        for i in self.__mp_price_column:
            ...
            for j, k in enumerate(self.__letters):
                if float(self.__mp_intervals[j][2]) <= float(i) <= float(self.__mp_intervals[j][1]):
                    interval_center = self.__find_mp_center_price(
                        int(self.__mp_intervals[j][1].replace('.', '')), int(self.__mp_intervals[j][2].replace('.', '')))
        ...
        for i, j in enumerate(self.__mp_price_column):
            ...
            for n, k in enumerate(self.__letters):
                if float(self.__mp_intervals[n][2]) <= float(j) <= float(self.__mp_intervals[n][1]):
    ...
    def __mark_mp_center(self) -> None:
        ...
        self.__market_profile[self.__mp_price_column.index(self.__profile_center[0])][8 + len(self.__mp_intervals)] = '-'
        self.__market_profile[self.__mp_price_column.index(self.__profile_center[1])][8 + len(self.__mp_intervals)] = '-'
    ...
    def __mark_mp_opening_and_closing(self) -> None:
        ...
        self.__market_profile[self.__mp_price_column.index(self.__mp_intervals[0][0])][3] = '@'
        self.__market_profile[self.__mp_price_column.index(self.__mp_intervals[0][0])][9 + len(self.__mp_intervals)] = '@'
        for i, j in enumerate(self.__market_profile[self.__mp_price_column.index(
                self.__mp_intervals[len(self.__mp_intervals) - 1][3])]):
            if j == LETTERS[len(self.__mp_intervals) - 1]:
                self.__market_profile[self.__mp_price_column.index(
                    self.__mp_intervals[len(self.__mp_intervals) - 1][3])][i] = '#'
    ...
    def __write_daily_profile_to_db(self) -> None:
        ...
        """
        profile_date = self.__mp_intervals[0][4][:10]
        ...
        with create_connection('C:/mp/mp_db.sqlite') as connection:
            ...
            data_tuple = self.__ticker, profile_date, self.__market_profile_as_string
